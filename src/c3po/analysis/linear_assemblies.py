...
def diagonal_corrected_assembly_activity(ic_vectors, Z):
    ...
    for i, vec in enumerate(ic_vectors):  # vec: [n_units]
        ...
        signed_magnitude = np.sign(signed_proj) * np.sqrt(np.abs(quadratic_form))
...
def compute_linear_assembly(spike_vector, activation_metric="squared"):
    """
    Reference:
    ...
    ic_vectors = np.dot(ica.components_, V)  # project ICs back to original unit space
    ...
    ic_assembly_activities = activation_metric_dictionary[activation_metric](
        ic_vectors, Z
    )
    ...
    return pc_vectors, eigenvalues, ic_vectors, ic_assembly_activities
